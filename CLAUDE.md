...
> **수치 근거 추적**, **회사별 양식 1:1 재현**, **섹터 분석 깊이(rNPV·PubMed 등)**
> 세 가지다.
>
> 근거 추적을 1순위로 두는 이유: "AI가 초안을 써준다"는 이미 전 경쟁사가 하는
> 얘기라 차별점이 못 된다. 반면 심사역이 IC에서 실제로 받는 질문은 "이 숫자
> 어디서 나왔냐"이고, 하나라도 못 대면 결국 모든 수치를 손으로 재확인하게 돼
> 자동화의 이득이 사라진다. 이 병목을 없애는 게 채택률을 가른다.
