 ...
 *
 * When no OpenRouter API key is configured, the platform still produces a
 ...
 * real API key (OPENROUTER_API_KEY) automatically switches to live generation.
 */
...
const DEMO_NOTICE =
  "> ⚙️ **데모 모드** — OpenRouter API 키가 설정되지 않아 샘플 콘텐츠로 생성되었습니다. " +
  "`OPENROUTER_API_KEY`를 `.env.local`에 입력하면 동일한 흐름으로 실제 AI가 본문을 작성합니다.\n\n";
...
function labeledBlocksFor(prompt: string): string | null {
  ...
  const filler: Record<string, string> = {
    ...
    점수: "62",
    코멘트:
      "- 강점: 명확한 문제 정의와 초기 레퍼런스 확보\n- 약점: 트랙션 규모가 작고 단위 경제성 미검증 (자료 부족)\n- 추가 확인: 최근 12개월 매출 추이, 핵심 인력 이력, 라운드 조건",
    우려사항:
