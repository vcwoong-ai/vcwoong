...
import { searchPatentsByApplicant, formatKiprisForPrompt } from "./kipris";
...
export interface BioExternalData {
  ...
  kiprisPatents: Awaited<ReturnType<typeof searchPatentsByApplicant>>;
  indication: string;
  companyQuery: string;
}
...
export async function fetchBioExternalData(
  ...
  const [pubmedArticles, clinicalTrials, fdaDrugs, kiprisPatents] = await Promise.allSettled([
    ...
    searchPatentsByApplicant(companyName, 10),
  ]).then((results) =>
    ...
    Awaited<ReturnType<typeof searchPatentsByApplicant>>,
  ];
...
export function formatExternalDataForPrompt(data: BioExternalData): string {
  ...
  if (data.kiprisPatents.length > 0) {
    sections.push(formatKiprisForPrompt(data.kiprisPatents, data.companyQuery));
  }
