...
export async function detectSectors(
  ...
  for (const [sector, keywords] of Object.entries(SECTOR_KEYWORDS)) {
    scores[sector] = keywords.filter((kw) =>
  ...
  const { data } = await callClaudeJSON<{
    ...
    system:
      "당신은 한국 스타트업 투자 전문가입니다. 주어진 정보를 바탕으로 스타트업을 정확히 분류합니다.",
    messages: [
      {
        role: "user",
        content: `다음 스타트업을 분류하세요.\n\n사업 요약: ${businessSummary}\n제품/서비스: ${products}\n키워드 기반 후보 섹터: ${top3.join(", ")}\n\n가능한 섹터: BIO, IT, DEEPTECH, MANUFACTURING, CONTENT, CONSUMER, FINTECH, CLIMATE, GENERAL\n\nJSON으로 응답:\n{ "primary": "BIO", "secondary": ["DEEPTECH"], "confidence": 0.85, "reasoning": "..." }\n\n참고: 융합 산업(예: AI 신약개발)은 secondary에 추가. confidence는 0~1 범위. 소비재/D2C는 CONSUMER, 콘텐츠/엔터는 CONTENT, 기후/탄소는 CLIMATE.`,
      },
    ],
    maxTokens: 512,
    temperature: 0.1,
