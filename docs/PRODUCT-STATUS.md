...
| KIPRIS 특허 | **동작** — `KIPRIS_API_KEY` 설정 시 BIO 에이전트 프롬프트 주입 |
| 미구현 | KIPRIS 자동 갱신·알림 |
...
| 재현 미리보기 (교체 구간 표시) | 동작 |
| PPTX 출력 | **동작** — 섹션별 슬라이드 생성 (`?format=pptx`) |
| PPTX 양식 1:1 재현 | 미구현 (DOCX만 원본 재현) |
| 원본 대비 렌더 이미지 비교 | 미구현 |
...
| 단계 | 상태 |
|------|------|
| 딜소싱 | `/sourcing` 인박스, 키워드 섹터 추정, AI 5축 스크리닝, 딜 전환, **메일 붙여넣기 파싱** |
...
| **팀 협업** | **동작** — 팀 생성·초대·딜·양식 공유 (`/settings#team`, Multi+ 플랜) |
| 미구현 | 연간 결제 |
...
1. PPTX 양식 1:1 재현 (현재 DOCX만 원본 재현, PPTX는 신규 생성)
2. KIPRIS API 키 없을 때 대체 데이터 소스
3. 딜소싱 IMAP/Webhook 자동 수집 (현재 붙여넣기)
4. 팀 딜/양식 공유 UI (딜·양식 상세 화면 토글)
5. 원본 대비 렌더 이미지 비교 (양식 QA)
