...
검증: `npm run test:deal-scoring`, `test:fund-analytics`, `test:dart`, `test:irr-calculator`, `test:portfolio-grade` (전부 `test:all`에 포함, API 키 불필요)
...
5. PPTX/보고서에 실제 이미지·차트 삽입 (업로드 자료의 이미지 재활용,
   AI 텍스트 생성 구조라 현재는 완전히 없음 — 표 변환·핵심 지표
   자동 차트는 2026-08-10에 추가됨)
6. **코드상 "동작"으로 표시된 기능도 실사용자 테스트 없이는 신뢰하기
