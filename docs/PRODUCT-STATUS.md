...
| 원본 대비 렌더 이미지 비교 | 미구현 (미리보기 QA 점수로 대체) |
...
| 단계 | 상태 |
|------|------|
| 딜소싱 | `/sourcing` 인박스, 키워드 섹터 추정, AI 5축 스크리닝, 딜 전환, 메일 붙여넣기, **Webhook 자동 인입** |
...
| **팀 협업** | **동작** — 팀 생성·초대·딜·양식 공유 + **상세 화면 토글** |
| 미구현 | 연간 결제 |
...
1. KIPRIS API 키 실시간 연동 프로덕션 검증
2. IMAP 폴링 데몬 (Webhook 대신 직접 수집)
3. 원본 대비 렌더 이미지 비교 (양식 QA)
4. 연간 결제
5. 팀 역할별 권한 (편집/조회 분리)
