...
function buildPrompt(deal: Record<string, unknown>): string {
  const formatAmount = (v: unknown) =>
    typeof v === "number" ? formatCurrency(v, "KRW") : "미제공";

  return `당신은 한국 벤처캐피탈 전문 투자심사역입니다. 아래 딜 정보를 바탕으로 공식적인 투자심사보고서(投資審査報告書)를 한국어로 작성해주세요.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!apiKey || apiKey === "your-openai-api-key-here") {
      ...
      const prompt = buildPrompt(deal as Record<string, unknown>);
